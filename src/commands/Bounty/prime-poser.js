...
export default {
    ...
    execute: withErrorHandling(async (interaction, config, client) => {
        ...
        let signature = '';
        if (gradeKey === 'or') {
            signature = '🚨 **EL GATO veut cette tête. Ne reviens pas les mains vides.**';
        } else if (gradeKey === 'argent') {
            signature = '🃏 **EL GATO compte sur toi, hombre.**';
        } else {
            signature = '🌵 **Un petit contrat pour se faire la main, amigo.**';
        }
        ...
        // Si le serveur a un salon dédié, on pourrait aussi y poster automatiquement
        // → à brancher plus tard via guildConfig.bountyChannel
    }, { command: 'prime-poser' })
};
